...
User-reported navbar/Help-tab styling pass, scoped narrowly to those two areas per this session's brief:

- **Navbar divider unified across tabs.** The Builder tab showed a 4px slate-blue divider (`.dashboard-body`'s own `border-top`) stacked directly under the navbar's separate 1px grey `border-bottom`; the Help tab has no `.dashboard-body` wrapper, so it only ever showed the thin grey line. Moved the divider onto `.navbar` itself (4px, slate blue) and deleted `.dashboard-body`'s `border-top` — now one shared line regardless of active tab.
- **Active-tab indicator overlaid on the divider, not a second line beside it.** The previous indicator was a `border-bottom` on `.nav-link` itself, which sits at the link's own box edge — a few pixels away from the navbar's own border — so the two rendered as two separate, slightly offset lines, in a gold (`#F0DC9A`) that read poorly against the Help tab's plain cream background before the divider unification above. Replaced with an absolutely-positioned `::after` on the active link, same 4px weight, `bottom: -4px` so it lands exactly on the navbar's own border segment under that tab.
- **App title (`.navbar-brand`) restyled to match the app's other "page title" treatment** (`.drawer-header` / the Help page's own h4): slate blue, uppercase, bold, and enlarged (16px → 22px) — it had been reading smaller than the Help page's own title, which is backwards for the app's actual title.
- **Title/tab-label baseline alignment.** `.navbar-brand` and `.navbar-nav .nav-link` are different font sizes, and the navbar's default `align-items: center` centered each independently, leaving their bottoms visibly unaligned. Overrode to `align-items: flex-end` on `.navbar` and its `.container-fluid`, with both text elements' own vertical padding/margin zeroed, so both sit on one shared bottom edge just above the divider.
- **Help page title left-alignment.** The page's `<h4>` reused `.drawer-header`, which centers itself within a 1280px `max-width` to match the filter drawer's own content — on a wide viewport this left it inset from the section cards below, which have no such constraint and start flush at the content area's edge. Split into a new `.help-title` class: same typographic treatment, no centering, so its left edge now lines up with the cards'.
- **Help page section-card headers ("1. Data", "2. Variables & Elements", ...) restyled to match the page title's palette** (slate blue, uppercase) at a smaller size (14px), replacing `bslib::card_header()`'s plain default text. Scoped to a new `.help-panel` wrapper class since `card_header()` isn't used anywhere else in the app, so this can't affect other cards.

No R reactive logic touched — CSS (`www/style.css`) and static UI markup (`R/ui_help.R`, one class rename + one wrapper class) only. Stylesheet cache-buster bumped (`?v=feat011-11`). Verified by starting the app and confirming the new classes render in the served HTML/CSS; the Chrome browser extension was not connected this session, so the result was not checked with a live screenshot — worth a visual pass before this branch is considered for merge.
