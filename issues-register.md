...
| **Status** | **Resolved — 2026-09-06 (CHG-063)** |
| **File(s)** | `README.md` |
...
| **Resolution** | **CHG-063 (2026-09-06).** `README.md` rewritten in full: installation now describes `renv::restore()` (not a manual `.tar.gz`/dependency list — `forestHelperR` has a real GitHub source in `renv.lock` since CHG-040) and the bundled-TTF/Google-Fonts font setup (not `extrafont::font_import()`); usage overview rewritten around the bottom-rail + drawer model and the first-visit wizard; a new Testing section documents the `testthat`/`shinytest2` suites and the `NOT_CRAN=true` gotcha (ISS-037); Known limitations trimmed to what's actually still open (ISS-028/029/030/039/040/046 — was ISS-002/004/011/012, all resolved); Project structure rewritten to match the DEC-004 `server/`+`R/` split and current `www/*.js` files; a new Deployment section documents the Posit Connect Cloud publish and `manifest.json`; Prerequisites/Conventions updated to match `CLAUDE.md`. |
...
| **Status** | **Resolved — CHG-064 (2026-09-06)** |
| **File(s)** | `R/ui_drawers.R`, `www/drawer.js`, `www/style.css` |
| **Description** | In addition to jumping between drawers via the rail icons, add a "Back"/"Next" button pair inside each open drawer (left/right, presumably bottom-anchored near the panel content) that steps to the adjacent drawer in rail order. Data (first in the sequence) only needs "Next"; Order (last) only needs "Back"; every drawer in between gets both. Export is excluded from the sequence entirely (a terminal action panel, not a guided step). |
| **Design notes** | User's own suggestion: reuse the Export drawer's pill button sizing (CHG-057/061) rather than a new size. Claude's initial recommendation (outline pill, not solid) was built first, then the user's own live follow-up replaced it entirely with small chevron-only circles (`<`/`>`, no text) for a lighter, easier-to-place control — see Resolution. |
| **Related** | CHG-057 (Export drawer redesign — the pill button styling the first pass borrowed from), CHG-061 (Export layout fix, same session this idea was raised in), CHG-064 (implementation) |
| **Resolution** | **CHG-064 (2026-09-06).** Implemented as small (40px) circular chevron-only buttons, positioned client-side entirely (`www/drawer.js`'s `updateDrawerNavButtons()`, driven by the same `drawer-open` message that already toggles the active panel) and firing the exact same `rail_key` Shiny input the rail buttons themselves use — no `server/drawers.R` changes needed. Two follow-up fixes from the user's live check, same session: (1) buttons repositioned from a fixed 16px screen-edge inset to anchor against the drawer's own centred 1280px content column on wide viewports (≥1400px), since the original position read as "a long way from the panels" on a wide monitor; (2) a lingering solid-fill "stuck" look after clicking, caused by a `:focus` rule never clearing (unlike a real Shiny actionButton, this hand-rolled button stays in the DOM across every drawer switch) — fixed by dropping `:focus` from the hover trigger (kept `:focus-visible` for keyboard users) and calling `.blur()` after each click. |
