...
| ISS-036 | Medium | `renv.lock` | `forestHelperR` recorded as `Source: "unknown"`; blocks `renv::restore()` | 🟡 Partially resolved — CHG-039 (manifest only, `renv.lock` still unfixed) |
...
| **Severity** | Medium |
| **Status** | **Partially resolved — CHG-039 (2026-09-06), for the Connect Cloud manifest only; `renv.lock` itself still unfixed, so still Open for that** |
| **File(s)** | `renv.lock`, `renv/cellar/forestHelperR_0.2.0.tar.gz` (new) |
...
| **Related** | PDEC-005 (package hosting), PDEC-006 (dependency declaration), CHG-037, CHG-039 |
| **Resolution** | **Partial (CHG-039, 2026-09-06):** committed `renv/cellar/forestHelperR_0.2.0.tar.gz` and reinstalled it via `renv::install()` from that path — this gives the **installed package** (and therefore anything inspecting the local library, including `rsconnect::writeManifest()`) a resolvable `"Cellar"` source, without editing `renv.lock`. This was enough to generate a working `manifest.json` for the Connect Cloud publish. **`renv.lock`'s own `forestHelperR` entry is untouched and still reads `"Source": "unknown"`** — deliberately, per `CLAUDE.md`'s convention reserving lockfile edits for the R 4.5.2 migration (DEC-006). If Connect Cloud's GitHub-integrated deploy does its own server-side `renv::restore()` against the committed lockfile (rather than trusting the committed `manifest.json`), it may hit the identical failure independently — not yet confirmed either way. A full fix (updating `renv.lock`'s `Source` field, or moving to option (1) above) would need the same kind of explicit convention-override authorization DEC-006 gave the migration. |
